...
// (e.g. "Rejected by: Manager"), `label` sits directly on the block
// ("Remarks:" / "HR commented:"), `text` is the quoted body.
...
function renderDetails(details: EmailDetail[]): string {
  const rows = details
    .map(
      (d) => `
        <tr>
          <td style="padding:6px 16px 6px 0;font-family:${FONT};font-size:14px;color:${MUTED};white-space:nowrap;vertical-align:top;">${escapeHtml(d.label)}</td>
...
function renderQuote(quote: EmailQuote): string {
  const heading = quote.heading
    ? `<p style="margin:20px 0 0;font-family:${FONT};font-size:14px;color:${TEXT};">${escapeHtml(quote.heading)}</p>`
    : "";
  return `${heading}
...
<body style="margin:0;padding:0;background:#f5f6f8;">
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:560px;background:#ffffff;border-radius:8px;">
          <tr>
            <td style="padding:32px 32px 28px;">
              ...
              <p style="margin:16px 0 0;font-family:${FONT};font-size:15px;line-height:1.55;color:${TEXT};">${escapeHtml(content.intro)}</p>
  ...
  const lines: string[] = [content.greeting, "", content.intro];
  if (content.details) {
    lines.push("");
  ...
  if (content.quote) {
    lines.push("");
    if (content.quote.heading) lines.push(content.quote.heading, "");
